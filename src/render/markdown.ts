...
import { getCapabilities, hyperlink } from "./hyperlinks.js";
import { markdownParser } from "./strikethrough.js";
import { visibleWidth, wrapTextWithAnsi } from "./wrap.js";

export interface RenderOptions {
	/** When false, never emit OSC 8 hyperlinks. When true, force them. Default: auto-detect. */
	hyperlinks?: boolean;
...
class MarkdownRenderer {
	...
	constructor(text: string, theme: MarkdownTheme, options?: RenderOptions) {
		this.text = text;
		this.theme = theme;
		...
		this.hyperlinks =
			options?.hyperlinks !== undefined ? options.hyperlinks : getCapabilities().hyperlinks;
	}
	...
	render(width: number): string[] {
		...
		const contentLines = wrappedLines.map((line) => leftMargin + line);
...
/** Render markdown source to styled terminal lines. */
export function renderMarkdown(
	...
	return new MarkdownRenderer(text, theme, options).render(width);
}

/**
 * Compatibility wrapper matching the pi TUI Markdown component constructor.
 */
export class Markdown {
	...
	render(width: number): string[] {
		return new MarkdownRenderer(this.text, this.theme, {
